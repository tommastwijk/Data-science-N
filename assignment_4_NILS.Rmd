...
author: |
  Nils van der Put 
  n.e.j.vanderput@students.uu.nl
  Nils-data-science
date: April 2023
urlcolor: purple
linkcolor: purple
...
data_question_2.1 <- GrowthSW %>% mutate(treat = ifelse(revolutions > 0, "More than 0", "0"))
D_2.1 <- datasummary(growth + rgdp60 ~ treat * (mean + median + sd + min + max), data = GrowthSW)
...
the median growth for countries with zero revolutions is bigger than the median growth for countries that had more than zero revolutions.
...
the significance level is 0.05. With a P-value of 0.06871 the null hypothesis is true. There is a 6.871% chance of observing a test statistic as extreme as the one obtained in the analysis.
...
It is for the average wealth in GDP per capita. It is there to make sure that as many variables as possible are included that may have an effect. 
...
save(model1, model2, model3, model4, file = "Q3.4")
...
I think it is tradeshare, because it has the highest values in comparison with the other variables. 
...
doc <- list(model1, model2, model3, model4) |>
  modelsummary(stars = TRUE, gof_map = c("nobs", "r.squared"))
output_file <- "worddoc3.8.docx"
